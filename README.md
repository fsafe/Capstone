...
The FPN is essentially a feature detector and not an object detector on its own. The features from the FPN are fed into a learned RPN. The RPN then learns to propose regions of interest (RoI) from the feature maps using anchors which are a set of boxes which scale according to the input image. These RoIs are regions which may contain an object.

RoIAlign:
Fast R-CNN and Faster R-CNN can be considered predecessors of Mask R-CNN. In both Fast/Faster R-CNN RoIPooling is used to extract small feature maps from each RoI window coming out of the RPN. It should be noted that quantizing is performed in Fast/Faster R-CNN both going from the input image to the RoI windows and also from the RoI to the small RoI feature maps created by RoIPooling. To understand quantizing take as an example if the input image is 800x800 with an RoI of 665x665 and the backbone CNN reduces the image and RoI by a factor of 32 then the dimention of the RoI projected on the feature map outputed by the backbone would be floor(665/32)=20. This is called quantizing a floating point number and causes information loss. Quantizing is also done during RoIPooling when going from the image feature map to the RoI feature map. To account for quantizing in a Mask R-CNN an RoIAlign layer is ued which removes the quantization done by RoIPool.
