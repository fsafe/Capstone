...
md5_dict = {}
with open('MD5_checksums.txt', 'rt') as md5_file:
    reader = csv.reader(md5_file, delimiter=' ', skipinitialspace=True)
    for row in reader:
        md5_dict[row[1]] = row[0]
zip_dir = 'Images_png'
filenames = []
with open('DL_info.csv', 'rt') as csvfile:
    reader = csv.reader(csvfile)
    next(reader, None)  # skip the headers
    for row in reader:
        filename = row[0]  # replace the last _ in filename with / or \
        idx = filename.rindex('_')
        filenames.append(zip_dir + '/' + filename[:idx] + '/' + filename[idx + 1:])
    key_slice_files = set(filenames)
    for idx, link in enumerate(links):
        ...
        assert hash_md5.hexdigest() == md5_dict[fn]
        print('Extracting key slices from', fn)
        with zipfile.ZipFile(fn, 'r') as zip_obj:
            zip_file_slices = set(zip_obj.namelist())
            zip_obj.extractall(members=zip_file_slices.intersection(key_slice_files))
